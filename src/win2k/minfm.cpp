...
#include "minfm.h"    // contains class definitions.
#include "patch.h"
#include "natv.h"
#include "bank.h"
...
// ==============================================================================
NTSTATUS CreateMiniportMidiESFM
(
...
//  expand STD_CREATE_BODY_ to take constructor(boolean) for whether to include volume
    NTSTATUS ntStatus;
    CMiniportMidiFM *p =  
        new(PoolType,'rCcP') CMiniportMidiFM(UnknownOuter);
...
)
{
    ...
    if (NT_SUCCESS(ntStatus))
    {
        ...
        //
        m_PortBase = PUCHAR((ULONG_PTR)ResourceList->FindTranslatedPort(0)->u.Port.Start.LowPart);
...
)
{
    ...
    //
    UnknownAdapter->QueryInterface (IID_IAdapterCommon, (PVOID *)&m_pAdapterCommon);
    NTSTATUS ntStatus = ProcessResources(ResourceList);

    if (NT_SUCCESS(ntStatus))
    {
        // NB: Not in original driver code but without started FM, detection
        // will always go wrong??
        m_pAdapterCommon->StartESFM(TRUE);
        ...
        if (SoundSynthPresent(m_PortBase, m_PortBase))
        {
            ...
            GetRegistrySettings();
#endif
            if (!m_fESFM && SoundMidiIsOpl3())
            {
                _DbgPrintF(DEBUGLVL_VERBOSE, ("[FM16::Init] Type = OPL3"));
            ...
            {
                _DbgPrintF(DEBUGLVL_TERSE, ("[FM16::Init] Type = ESFM"));
                _DbgPrintF(DEBUGLVL_TERSE, ("[FM16::Init] base = %X", m_PortBase));
                m_fESFM = TRUE;
                WRITE_PORT_UCHAR(m_PortBase + 2, 5);
                KeStallExecutionProcessor(25);
                WRITE_PORT_UCHAR(m_PortBase + 1, 0x80);
                KeStallExecutionProcessor(25);
                g_PortBase = m_PortBase;
                fmreset();
            }
        }
        else
        {
            _DbgPrintF(DEBUGLVL_TERSE, ("[FM16::Init] SoundSynthPresent failed"));
            ntStatus = STATUS_DEVICE_CONFIGURATION_ERROR;
        }
    }
    else
    {
        _DbgPrintF(DEBUGLVL_TERSE, ("[FM16::Init] ProcessResources failed"));
    }

    // NB: Not present in original source
    if (!NT_SUCCESS(ntStatus))
    {
        //
        // clean up our mess
...
)
{
    ...
    if (m_Miniport) m_Miniport->m_pAdapterCommon->StartESFM(FALSE);
    Opl3_AllNotesOff();
...
)
{
    ...
    m_Miniport = Miniport;
    m_Miniport->AddRef();
    m_Miniport->m_pAdapterCommon->StartESFM(TRUE);
...
)
{
    ...
    _DbgPrintF(DEBUGLVL_VERBOSE, ("[MidiFM::SoundMidiSendFM]"));
    _DbgPrintF(DEBUGLVL_VERBOSE, ("[SoundMidiSendFM] Writing Address 0x%X ", Address));
    WRITE_PORT_UCHAR(PortBase + (Address < 0x100 ? 0 : 2), (UCHAR)Address);
    KeStallExecutionProcessor(23);

    _DbgPrintF(DEBUGLVL_VERBOSE, ("[SoundMidiSendFM] Writing Data 0x%X to ", Data));
...
)
{
    ...
    if (Length <= 4)
    {
        RtlCopyMemory(&Length, BufferAddress, Length);
        if (Length & 0x80)
        {
            if (m_Miniport->m_fESFM)
                MidiMessage(Length);
            else
                WriteMidiData(Length);
        }
    }
    *BytesWritten = Length;
...
SoundMidiIsOpl3(void)
{
    ...
    if (SoundSynthPresent(m_PortBase + 2, m_PortBase))
    {
        ...
        if (!SoundSynthPresent(m_PortBase + 2, m_PortBase))
        {
            _DbgPrintF(DEBUGLVL_VERBOSE, ("[FM16: In SoundMidiIsOpl3] right half disappeared"));
    ...
    _DbgPrintF(DEBUGLVL_VERBOSE, ("[FM16: In SoundMidiIsOpl3] returning bIsOpl3 = 0x%X", bIsOpl3));
    return(bIsOpl3);
}

#pragma code_seg()
VOID  CMiniportMidiFM::
SoundMidiQuiet(void)
/*++
...
--*/
{
	...
	SoundMidiSendFM (m_PortBase, AD_NEW, 0x01);
	SoundMidiSendFM (m_PortBase, AD_MASK, 0x60);
	SoundMidiSendFM (m_PortBase, AD_CONNECTION, 0x3f);
	SoundMidiSendFM (m_PortBase, AD_NTS, 0x00);
...
)
{
    ...
    SoundMidiSendFM(base, AD_MASK, 0x60);             // mask T1 & T2
    SoundMidiSendFM(base, AD_MASK, 0x80);             // reset IRQ

    t1 = READ_PORT_UCHAR((PUCHAR)inbase);       // read status register

    SoundMidiSendFM(base, AD_TIMER2, 0xff);             // set timer - 1 latch
    SoundMidiSendFM(base, AD_MASK, 0x21);             // unmask & start T1
    ...
    SoundMidiSendFM(base, AD_MASK, 0x60);
    SoundMidiSendFM(base, AD_MASK, 0x80);
...
WriteMidiData(DWORD dwData)
{
    ...
    _DbgPrintF(DEBUGLVL_VERBOSE,("[MidiFM::WriteMidiData]"));
    ...
    switch (bMsgType)
    {
        ...
        case 0x80:
            ...
            if (bChannel == DRUMCHANNEL)
            {
                Opl3_NoteOff((BYTE) (bNote + 128),bNote, bChannel);
            }
            else
            {
...
Opl3_AllNotesOff()
{
    ...
    {
        Opl3_NoteOff(m_Voice[i].bPatch, m_Voice[i].bNote, m_Voice[i].bChannel);
    }
}
...
// Custom extension to load different patch-set
#ifdef LOAD_PATCHES
/*****************************************************************************
...
)
{
    ...
    if(NT_SUCCESS(ntStatus))
    {
        ...
        PVOID KeyInfo = ExAllocatePool(PagedPool, sizeof(KEY_VALUE_PARTIAL_INFORMATION) + sizeof(WCHAR) * MAX_PATH);
        if(NULL != KeyInfo)
        {
            RtlInitUnicodeString( &KeyName, INI_STR_PATCHLIB );
            ResultLength = 0;
            ...
            if(NT_SUCCESS(ntStatus))
            {
                ...
                // If this is a valid bank file with appropriate size, read it and overwrite internal bank data
                if (PartialInfo->Type == REG_SZ)
                {
                    ...
                    RtlInitUnicodeString(&UnicodeString, (PCWSTR)PartialInfo->Data);
                    ...
                    if (NT_SUCCESS(status))
                    {
                            ...
                            FileStandardInformation)))
                        {
                            ...
                            if (FileStandardInformationBlock.EndOfFile.LowPart == sizeof(bank))
                            {
                                ZwReadFile(hFile,
                                    NULL,
                                    NULL,
...
#if 0  // NB: Not really neaded, StartESFM() puts us in ESFM-mode anyway!
            RtlInitUnicodeString(&KeyName, L"ForceESFM");
            ResultLength = 0;
            ...
            ntStatus = DriverKey->QueryValueKey(&KeyName,
                KeyValuePartialInformation,
                KeyInfo,
                sizeof(KEY_VALUE_PARTIAL_INFORMATION) + sizeof(BOOL),
                &ResultLength);
            if (NT_SUCCESS(ntStatus))
            {
                PartialInfo = PKEY_VALUE_PARTIAL_INFORMATION(KeyInfo);
